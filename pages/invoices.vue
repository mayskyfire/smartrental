<template>
  <div>
    <div class="flex flex-col gap-3 mb-4 sm:mb-6">
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <button
          ...
          class="px-3 sm:px-4 py-2.5 sm:py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2"
        >
          <svg
            class="w-4 h-4"
            fill="none"
        ...
        <button
          @click="sendBatch"
          class="px-3 sm:px-4 py-2.5 sm:py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 flex items-center justify-center gap-2"
        >
          <svg
            class="w-4 h-4"
            fill="none"
    ...
    <div class="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 sm:mb-6">
      ...
      <button
        @click="loadInvoices"
        class="mt-3 sm:mt-4 w-full sm:w-auto px-3 sm:px-4 py-2.5 sm:py-2 bg-gray-600 text-white text-sm rounded-lg hover:bg-gray-700"
      >
        ค้นหา
      </button>
    </div>
    ...
    >
      ...
      <div v-else-if="!invoices.length" class="p-8 text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">
          ลองสร้างใบแจ้งหนี้สำหรับเดือนนี้
        </p>
        <div class="mt-6">
          <button
            @click="generateMonthly"
        ...
        >
          <div class="flex justify-between items-start mb-2">
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-gray-500 mt-0.5">
                {{ invoice.property.name }} - {{ invoice.unit.unitCode }}
              </p>
            </div>
          ...
          <div class="space-y-1 text-xs mb-3">
            <div>
              ...
              {{ invoice.billingMonth }}/{{ invoice.billingYear + 543 }}
            </div>
            <div>
            ...
            <div>
              <span class="text-gray-500">ยอดสุทธิ:</span> ฿{{
            ...
            <div>
              <span class="text-gray-500">คงค้าง:</span>
          ...
          <div class="flex gap-2">
            <button
              ...
              class="flex-1 px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700"
            >
              บันทึกชำระ
            </button>
            <button
    ...
    <div
      ...
      class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center min-h-screen px-4 py-6 bg-black bg-opacity-50"
    >
...
async function loadInvoices() {
  ...
  try {
    ...
    const res: any = await $fetch("/api/invoices", { params: query });
...
async function sendBatch() {
  const confirmed = await showConfirm(
    "ส่งแจ้งเตือน LINE ให้ผู้เช่าทั้งหมด?",
    "ยืนยันการส่ง",
    "info"
  );
  if (!confirmed) return;

  const res: any = await $fetch("/api/line/send-monthly-batch", {
    method: "POST",
    body: {
...
onMounted(async () => {
  const res: any = await $fetch("/api/properties");
